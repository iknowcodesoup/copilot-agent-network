import { log, store, uid } from "@/lib/store"
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  log(voice.runIds[voice.runIds.length - 1] ?? voice.id, "info", "sample", `Synthesized sample: "${text.slice(0, 48)}"`)
