...
- source_spec: `_bmad-output/implementation-artifacts/spec-3-6-manage-voice-training-from-the-voices-view.md`
  summary: The factory control API (`star-trek-voyicer/apps/jeanlucrecord/api.py`) has no endpoint to fetch or stat a voice's exported `.onnx` model file — `stage_export` only writes it to a local `output/{character}.onnx` path on the GPU host. The Voices view's "model size once READY" and "Download model" (per epic-3-context.md) need a new `GET /characters/{character}/model` (+ `/model/download`) endpoint on the factory, a matching `VoiceFactoryGateway` method (mirroring `stream_clip_audio`), and a proxied `pythonapi` route.
  evidence: Confirmed by reading `api.py`: no route under `/characters/{character}/model*` exists, and `main.py`'s `stage_export` writes only to the local filesystem. Split out of Story 3.6 at spec-drafting time — the full spec was ~2,400-2,500 tokens against a 1,600 ceiling, and this backend plumbing was the largest single contributor. Story 3.6 ships with "Download model" disabled/hidden and no size shown until this lands.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-6-manage-voice-training-from-the-voices-view.md`
  summary: Voice (training-phase) state changes are never published to the existing Redis-backed voice-event SSE stream — `VoiceEvent.data` in `core/voice_events.py` only accepts `VoiceRun | VoiceLogChunk`, and `VoiceTrainingReconciler` (`workers/voice_training_reconciler.py`) has no `event_stream` dependency at all, unlike its sibling `VoiceRunReconciler`. The Voices view therefore has no live-push channel for phase changes; a card only reflects a new phase after a manual refetch (e.g. after calling "Train now"/"Retrain" or reloading the page).
  evidence: Confirmed by reading `voice_events.py`'s `VoiceEvent` union type, `voice_training_reconciler.py`'s constructor (no `event_stream` param), and `main.py`'s `VoiceTrainingReconciler(...)` construction (no `event_stream=` arg, unlike `voice_run_reconciler` a few lines above). Split out of Story 3.6 at spec-drafting time as the second-largest contributor to the oversized spec. Story 3.6 ships with refetch-on-mutation only, not live push.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-6-manage-voice-training-from-the-voices-view.md`
  summary: `phase_badge.tsx`'s `phaseStyles` and `phaseLabelsByPhase` maps have no runtime fallback for a phase value absent from both `VoiceRunPhase` and `VoicePhase` — if the backend ever adds an enum member before the frontend redeploys with the matching union update, `phaseStyles[phase]`/`phaseLabelsByPhase[phase]` return `undefined` and the badge renders with no visible label or styling.
  evidence: Round review (blind-hunter, edge-case-hunter) both flagged this independently. Confirmed by reading `phase_badge.tsx`: `Record<AnyPhase, string>` guarantees compile-time coverage of the known TypeScript unions but not runtime safety against live API drift. Pre-existing pattern — the file had the identical gap for `Record<VoiceRunPhase, string>` before this story, which only widened the same map to `AnyPhase` rather than introducing the underlying risk.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-flatten-assign-commit.md`
  summary: No component/unit test exercises the merged-endpoint UI wiring end to end — `SpeakerBoard.assignAndTrain`'s payload construction, `AssignSpeakerTool`'s chat-tool handler, or `VoiceSpeakerCombobox`'s "no free-text entry" guarantee (Acceptance Criterion 3). All three are asserted only by backend HTTP-contract tests and manual review.
  evidence: Verification-gap review confirmed via grep that none of the three existing voices-feature test files (`voice_api.test.tsx`, `voice_card.test.tsx`, `voices_view.test.tsx`) render `SpeakerBoard`, call `useAssignRun`, or import `voice_copilot_actions.tsx`/`voice_speaker_combobox.tsx`. The spec's own Verification section only required the listed CLI test/lint commands, which passed — this is a real coverage gap but not a blocker this pass per the spec's stated bar.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-flatten-assign-commit.md`
  summary: `assign_run`'s multi-write sequence (contribution rows, then per-voice phase update + reconciler wake, then run phase update) has no transaction or rollback — a failure partway through (e.g. the 2nd of 3 voice updates) leaves contribution rows persisted, some voices in TRAINING, and the run still AWAITING_REVIEW, with no `discard_run` left to unwind it.
  evidence: Edge-case-hunter flagged this. Confirmed by reading `routes/voice.py`'s `assign_run`: no `async with session.begin()` or equivalent wraps the per-contribution/per-voice loop. Pre-existing shape — the old `commit_run` had the identical non-atomic multi-write sequence — but the removal of `discard_run` (an explicit, confirmed spec decision) means there is now no manual escape hatch for a run left in this stuck state, which is a real behavior change worth a follow-up.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-flatten-assign-commit.md`
  summary: `assign_run` does not reject a request that maps two different speaker labels to the same `voice_id`, nor does it validate that `assign_request.assignments` keys match the run's actual known speaker labels.
  evidence: Edge-case-hunter flagged this. Confirmed by reading `routes/voice.py`: the assignment loop only checks each `voice_id` exists, never checks for duplicate targets or unknown speaker-label keys. Low real-world likelihood (the UI only ever submits speaker labels it read from the run's own speaker list) but the route itself has no server-side guard against a malformed direct API call.
